...
*/
#include<deque>
namespace BATreeNode{
    //内部节点
    template<typename K,typename E>
    class BAddTreeNode{
        public:
            //只能作为根节点创建
            explicit BAddTreeNode();//默认构造函数，创建一个空Btree，初始化时有0个关键码及一个空孩子分支
            ~BAddTreeNode();//析构不释放孩子内存，仅仅释放节点e内存
            bool isLeaf()const;//判断节点是否为叶子节点
        public:
            BAddTreeNode<K,E>* parent;//父节点
            std::deque<K>key;//关键码
            std::deque<BAddTreeNode<K,E>*>child;//孩子分支，总比关键字码多1
    };
    template<typename K,typename E>
